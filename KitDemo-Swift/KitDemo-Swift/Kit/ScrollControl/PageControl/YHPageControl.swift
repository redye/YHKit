...
public class YHPageControl: UIControl {
    ...
    @IBInspectable public var imageWidth: Int = 10
    
    private var contentWidth: Int {
        return (imageWidth + spacing) * numberOfPage
    }
    ...
    private func layoutUI() {
        ...
        for (index, imageView) in imageViews.enumerate() {
            ...
            imageView.layer.borderColor = pageIndicatorTintColor.CGColor
    ...
    private func handleTouches(touches: Set<UITouch>, withEvent event: UIEvent?) {
        if let touch = touches.first {
            ...
            if position.x > CGRectGetMaxX(contentView.frame) {
                self.currentPage += 1
            } else if (position.x < CGRectGetMinX(contentView.frame)) {
                self.currentPage -= 1
            }
        }
    }
